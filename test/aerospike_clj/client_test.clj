...
(defn- running-in-ci?
  "CI workflows in GitHub Actions always have the env var CI set to `true`.
  See: https://docs.github.com/en/actions/reference/environment-variables#default-environment-variables"
  []
  (boolean (System/getenv "CI")))

(def host (if (running-in-ci?)
...
(deftest client-creation
  (let [c (client/init-simple-aerospike-client [host] "test")]
    (is c)
    (is (= host (:cluster-name c))))

  (letfn [(no-password? [ex]
            (let [conf (:conf (ex-data ex))]
              (and conf (not (contains? conf "password")))))]
    (let [ex (is (thrown-with-msg? Exception #"unbounded delay queue" (client/init-simple-aerospike-client [host] "test" {"maxCommandsInProcess" 1})))]
      (is (no-password? ex)))

    (with-redefs [client/create-event-loops (constantly nil)]
      (let [ex (is (thrown-with-msg? Exception #"event-loops" (client/init-simple-aerospike-client [host] "test")))]
        (is (no-password? ex))))))
